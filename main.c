# include "libasm.h"

void check_strlen()
{
	char *empty = "";
	char *hello_world = "Hello world !";
	char *alphabet = "abcdefghijklmnopqrstuvwxyzgvvnbjchkjbn gcjhgkjbnbcgcfhgvjkj";

	printf("\n================================\n");
	printf("========== FT_STRLEN ===========\n");
	printf("================================\n\n");
	printf("----------CADENA VACIA------------\n");
	printf("%-20s: %zu\n", "libc--", strlen(empty));	
	printf("%-20s: %zu\n", "libasm", ft_strlen(empty));
	printf("\n");
	printf("----------HELLO WORLD !-----------\n");
	printf("%-20s: %zu\n", "libc--", strlen(hello_world));	
	printf("%-20s: %zu\n", "libasm", ft_strlen(hello_world));
	printf("\n");
	printf("----------CADENA LARGA------------\n");
	printf("%-20s: %zu\n", "libc--", strlen(alphabet));	
	printf("%-20s: %zu\n", "libasm", ft_strlen(alphabet));
	printf("\n");
	
	//printf("----------SEG FAULT------------\n");
	// printf("%-20s: %zu\n", "libc", strlen(NULL));	
	// printf("%-20s: %zu\n", "libasm", ft_strlen(NULL));
	printf("\n");
}

void clear_buffer(char *buffer, int size)
{
	int i = 0;
	while (i < size)
		buffer[i++] = 0;
}

void check_strcpy()
{
	char buffer[30];
	
	char *empty = "";
	char *hello_world = "Hello world !";
	char *alphabet = "abcdefghijklmnopqrstuvwxyzr";
	
	printf("\n================================\n");
	printf("========== FT_STRCPY ===========\n");
	printf("================================\n\n");
	printf("----------CADENA VACIA------------\n");
	printf("%-20s: \"%s\"\n", "libc--", strcpy(buffer, empty));	
	clear_buffer(buffer, 30);
	printf("%-20s: \"%s\"\n", "libasm", ft_strcpy(buffer, empty));	
	clear_buffer(buffer, 30);
	printf("\n");
	printf("----------HELLO WORLD !-----------\n");
	printf("%-20s: \"%s\"\n", "libc--", strcpy(buffer, hello_world));	
	clear_buffer(buffer, 30);
	printf("%-20s: \"%s\"\n", "libasm", ft_strcpy(buffer, hello_world));	
	clear_buffer(buffer, 30);
	printf("\n");
	printf("----------CADENA LARGA------------\n");
	printf("%-20s: \"%s\"\n", "libc--", strcpy(buffer, alphabet));	
	clear_buffer(buffer, 30);
	printf("%-20s: \"%s\"\n", "libasm", ft_strcpy(buffer, alphabet));
	clear_buffer(buffer, 30);
	printf("\n");

	//printf("----------SEG FAULT------------\n");
	// printf("%-20s: \"%s\"\n", "libc", strcpy(NULL, NULL));	
	// clear_buffer(buffer, 30);
	// printf("%-20s: \"%s\"\n", "libasm", ft_strcpy(NULL, NULL));
	// clear_buffer(buffer, 30);
	printf("\n");
}

void check_strcmp()
{
	char *empty1 = "";
	char *empty2 = "";
	char *hello_world = "Hello world !";
	char *hello_human = "Hello human !";
	char *hello_world2 = "Hello world !";
	
	printf("\n================================\n");
	printf("========== FT_STRCMP ===========\n");
	printf("================================\n\n");
	printf("----------CADENAS VACIAS------------\n");
	printf("%-20s: \"%d\"\n", "libc--", strcmp(empty2, empty1));
	printf("%-20s: \"%d\"\n", "libasm", ft_strcmp(empty2, empty1));
	printf("\n");
	printf("----------1º CADENA VACIA------------\n");
	printf("%-20s: \"%d\"\n", "libc--", strcmp(empty1, hello_world2));
	printf("%-20s: \"%d\"\n", "libasm", ft_strcmp(empty1, hello_world2));
	printf("----------2º CADENA VACIA------------\n");
	printf("%-20s: \"%d\"\n", "libc--", strcmp(hello_world2, empty1));
	printf("%-20s: \"%d\"\n", "libasm", ft_strcmp(hello_world2, empty1));
	printf("----------CADENAS IGUALES------------\n");
	printf("%-20s: \"%d\"\n", "libc--", strcmp(hello_world, hello_world2));
	printf("%-20s: \"%d\"\n", "libasm", ft_strcmp(hello_world, hello_world2));
	printf("\n");
	printf("----------CADENAS DIFERENTES------------\n");
	printf("%-20s: \"%d\"\n", "libc--", strcmp(hello_world, hello_human));
	printf("%-20s: \"%d\"\n", "libasm", ft_strcmp(hello_world, hello_human));
	printf("\n");

	/*printf("----------SEG FAULT------------\n");
	 printf("%-20s: \"%d\"\n", "libc", strcmp(NULL, hello_world2));
	 printf("%-20s: \"%d\"\n", "libasm", ft_strcmp(NULL, empty1));
	printf("\n");*/

	/*printf("----------SEG FAULT2------------\n");
	 printf("%-20s: \"%d\"\n", "libc", strcmp(hello_world2, NULL));
	 printf("%-20s: \"%d\"\n", "libasm", ft_strcmp(hello_world2, NULL));
	printf("\n");*/
}

void check_write()
{
	char *hello_world = "hello world!\n";
	char *empty = "";
	int fd;
	int fd2;
	fd = open("archivo.txt", O_WRONLY);
	fd2 = open("inexistente.txt", O_WRONLY);

	printf("\n================================\n");
	printf("========== FT_WRITE ============\n");
	printf("================================\n\n");

	printf("----------HELLLO WORLD!------------\n");
	printf("%-20s: \"Libc:  %zu\"\n", "libc  ", write(1, hello_world, 13));
	printf("\n");
	printf("%-20s: \"Libasm:%zu\"\n", "libasm", ft_write(1, hello_world, 13));
	printf("\n");
	printf("----------CADENA VACIA------------\n");
	printf("%-20s: \"Libc:  %zu\"\n", "libc  ", write(1, empty, 0));
	printf("\n");
	printf("%-20s: \"Libasm:%zu\"\n", "libasm", ft_write(1, empty, 0));
	printf("\n");
	printf("----------NULL------------\n");
	printf("%-20s: \"Libc:  %zu\"\n", "libc  ", write(-7, NULL, 7));
	printf("\n");
	printf("%-20s: \"Libasm:%zu\"\n", "libasm", ft_write(-7, NULL, 7));
	printf("----------FD INCORRECTO------------\n");
	printf("%-20s: \"Libc:  %zu\"\n", "libc  ", write(8, hello_world, 13));
	printf("\n");
	printf("%-20s: \"Libasm:%zu\"\n", "libasm", ft_write(8, hello_world, 13));
	printf("----------FD EXISTENTE------------\n");
	printf("%-20s: \"Libc:  %zu\"\n", "libc  ", write(fd, hello_world, 13));
	printf("\n");
	printf("%-20s: \"Libasm:%zu\"\n", "libasm", ft_write(fd, hello_world, 13));
	printf("----------FD INEXISTENTE------------\n");
	printf("%-20s: \"Libc:  %zu\"\n", "libc  ", write(fd2, hello_world, 13));
	printf("\n");
	printf("%-20s: \"Libasm:%zu\"\n", "libasm", ft_write(fd2, hello_world, 13));
	printf("\n");
	printf("----------FD ERRONEO------------\n");
	printf("%-20s: \"Libc:  %zu\"\n", "libc  ", write(-1, hello_world, 13));
	printf("\n");
	printf("%-20s: \"Libasm:%zu\"\n", "libasm", ft_write(-1, hello_world, 13));
	printf("\n");
	
}

void check_read()
{
	int fd = open("ft_read.s", O_RDONLY);
	char buff1[891];
	int ret = 1;
	printf("\n================================\n");
	printf("========== FT_READ =============\n");
	printf("================================\n\n");
	printf("----------FT_READ.S TEST1-----------\n");
	printf("%-20s: \n", "libc-- ");
	ret = read(fd, buff1, 890);
	buff1[ret] = 0;
	printf("[return : %d]\n|%s|\n", ret, buff1);
	printf("\n");
	close(fd);
	fd = open("ft_read.s", O_RDONLY);
	clear_buffer(buff1, 891);
	printf("%-20s: \n", "libasm ");
	ret = ft_read(fd, buff1, 890);
	buff1[ret] = 0;
	printf("[return : %d]\n|%s|\n", ret, buff1);
	printf("\n");
	clear_buffer(buff1, 891);
	close(fd);

	printf("----------TEST TEST2------------\n");
	fd = open("test", O_RDONLY);
	printf("%-20s: \n", "libc-- ");
	ret = read(fd, buff1, 890);
	buff1[ret] = 0;
	printf("[return : %d]\n|%s|\n", ret, buff1);
	printf("\n");
	close(fd);
	fd = open("test", O_RDONLY);
	clear_buffer(buff1, 891);
	printf("%-20s: \n", "libasm ");
	ret = ft_read(fd, buff1, 890);
	buff1[ret] = 0;
	printf("[return : %d]\n|%s|\n", ret, buff1);
	printf("\n");
	clear_buffer(buff1, 891);
	close(fd);

	printf("----------WRONG TEST3-----------\n");
	fd = open("wrong", O_RDONLY);
	printf("%-20s: \n", "libc-- ");
	ret = read(fd, buff1, 890);
	buff1[ret] = 0;
	printf("[return : %d]\n|%s|\n", ret, buff1);
	printf("\n");
	close(fd);
	fd = open("wrong", O_RDONLY);
	clear_buffer(buff1, 891);
	printf("%-20s: \n", "libasm ");
	ret = ft_read(fd, buff1, 890);
	buff1[ret] = 0;
	printf("[return : %d]\n|%s|\n", ret, buff1);
	printf("\n");
	clear_buffer(buff1, 891);
	close(fd);
}

void check_strdup()
{
	char *hello_world = "afdasffnoijdsfknnfois";
	char *empty = "";
	char *save;
	char *save2;
	
	printf("\n================================\n");
	printf("========== FT_STRDUP ===========\n");
	printf("================================\n\n");
	printf("----------CADENA LARGA------------\n");
	save = strdup(hello_world);
	printf("%-20s: \"%s\"\n", "libc--", save);
	free(save);
	save = NULL;
	save2 = ft_strdup(hello_world);
	printf("%-20s: \"%s\"\n", "libasm", save2);
	free(save2);
	save2 = NULL;
	printf("\n");

	printf("----------EMPTY------------\n");
	save = strdup(empty);
	printf("%-20s: \"%s\"\n", "libc--", save);
	free(save);
	save = NULL;
	save2 = ft_strdup(empty);
	printf("%-20s: \"%s\"\n", "libasm", save2);
	free(save2);
	save2 = NULL;
	printf("\n");

	// ------- NULL = SEGFAULT
	// printf("%-20s: NULL\n", "char *");
	// save = strdup(NULL);
	// printf("%-20s: \"%s\"\n", "libc", save);
	// free(save);
	// save = NULL;
	// save2 = ft_strdup(NULL);
	// printf("%-20s: \"%s\"\n", "libasm", save2);
	// free(save2);
	// save2 = NULL;
	// printf("\n");
}

int main()
{
	/*check_strlen();
	check_strcpy();
	check_strcmp();
	check_write();
	check_read();
	check_strdup();*/
	return (0);
}
